            ...
            {posts.map((post) => (
              ...
              >
                <Link href={`${post.tech}`}>
                  <div className="">
                    <div
                      className="absolute w-[3vw] h-[3vw] rounded-full pointer-events-none transition-transform duration-300 ease-out bg-gradient-to-r from-purple-500 to-blue-500 flex flex-col leading-none gap-0 items-center justify-center text-[1.2vw] font-[500] pb-8 z-20"
                      style={{
                        transform: `translate(${pos.x - 32}px, ${pos.y - 32}px) scale(${hovering ? 1 : 0})`,
                      }}
                    />
                    {/* Image displayed instead of Video */}
                    <Image
                      src={post.video || ""} // Use post.image or default image as fallback
                      alt={post.title}
                      width={1200}
                      height={720}
                    ...
                    <h3 className="md:text-[1.2vw] text-[1.2vh] text-zinc-200 text-start md:pt-[1vw] pt-[1vw]">
                      {post.title}
                    </h3>
                  </div>
                </Link>
